...
class RecipeController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            $recipes = MfgRecipe::join('variations as v', 'mfg_recipes.variation_id', '=', 'v.id')
                                ...
                                ->leftjoin('child_category as c', 'p.category_id', '=', 'c.id')
                                ->leftjoin('subcategory as sc', 'p.sub_category_id', '=', 'sc.id')
            ...
            return Datatables::of($recipes)
                ->addColumn('action', '<button data-href="{{action(\'\Modules\Manufacturing\Http\Controllers\RecipeController@show\', [$id])}}" class="btn btn-xs btn-info btn-modal btn-edt" data-container=".view_modal"><i class="fa fa-eye"></i></button>
                            &nbsp; @can("manufacturing.edit_recipe") 
                            <a href="{{action(\'\Modules\Manufacturing\Http\Controllers\RecipeController@addIngredients\')}}?variation_id={{$product_id}}" class="btn btn-xs btn-primary btn-vew" ><i class="fa fa-edit"></i></a>
                    &nbsp; 
                    <button data-href="{{action(\'\Modules\Manufacturing\Http\Controllers\RecipeController@destroy\',[$id])}}" class="btn btn-xs btn-danger delete_recipe btn-dlt"><i class="fa fa-trash"></i></button> @endcan')
                ...
                ->addColumn('unit_cost', function ($row) {
                    ...
                    $unit_cost = ($price / !empty($row->total_quantity) ? $row->total_quantity : 1);
    ...
    public function create()
    {
        ...
        $category = Type::where('name','FINISHED GOODS')->get();
        return view('manufacturing::recipe.create')->with(compact('recipes','category'));
    }

    public function get_product($cat_id){
        $data = Product::where('product_type',$cat_id)->get();
        return $data;
    }
        ...
        try {
            ...
            if (!empty($input['ingredients'])) {
                ...
                foreach ($input['ingredients'] as $key => $value) {
                    ...
                    $ingredient->product_id = $variation->product_id;
    ...
    public function addIngredients()
    {
        ...
        $variation = Variation::join('products as p', 'p.id', '=', 'variations.product_id') 
                                ...
                                // old
                                ->where('variations.product_id',$variation_id)
                                // new
                                // ->where('p.id',$variation_id)
                                ->select('p.name as product_name', 'p.type as product_type', 'variations.*', 'pv.name as product_variation_name', 'p.unit_id as unit_id', 
                                         'u.short_name as unit_name','p.weight as gross_weight','p.product_custom_field1 as net_weight')
                                // ->limit(10)->get();
                                // ->findOrFail($variation_id);
                                ->first();
        ...
        //new
        $recipe = MfgRecipe::where('product_id', $variation_id)->with($with)->first();
        // dd($recipe);
        $copy_recipe = null;
    ...
    public function getRecipeDetails()
    {
        ...
        // $recipe = MfgRecipe::where('product_id', $variation_id)
        ...
        $ingredient_table = view('manufacturing::recipe.ingredients_for_production', compact('ingredients','product','recipe', 'pos_settings', 'manufacturing_settings'))->render();
    ...
    public function getRecipeDetailsMulty()
    {
        $business_id = request()->session()->get('user.business_id');
        if (!(auth()->user()->can('superadmin') || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'manufacturing_module')) || !auth()->user()->can('manufacturing.access_recipe')) {
            abort(403, 'Unauthorized action.');
        ...
        $ingredient_table = view('manufacturing::recipe.ingredients_for_production_multy', compact('ingredients','product','recipe', 'pos_settings', 'manufacturing_settings'))->render();
    ...
    public function loadCsv(Request $request)
    {
        try {    
            ...
            if ($request->hasFile('csv')) {
                ...
                //     if($value[0] != null){
                //         $product_id = Product::where('sku',$value[0])->first();
                //         $variation_id = Variation::where('sub_sku',$value[0])->first();
                        
                //         $query = MfgRecipe::where('product_id', $product_id->id)->get();
                //         if($query->count() > 0){    
                //             $is_valid =  false;
                //             $error_msg = "Duplicate Product on row no. $row_no";
                //             break;
                //         }
                        
                        
                //         $mfg_recipe = new MfgRecipe;
                //         $mfg_recipe->product_id = $product_id->id;
                //         $mfg_recipe->variation_id = $variation_id->id;
                //         $mfg_recipe->save();
                //     }else{
                //         if($value[1] != null){
                //             $variation_ids = Variation::where('sub_sku',$value[1])->first();

                //             $mfg_recipe_ingredient = new MfgRecipeIngredient;
                //             $mfg_recipe_ingredient->mfg_recipe_id = $mfg_recipe->id;
                //             $mfg_recipe_ingredient->variation_id=$variation_ids->id;
                //             $mfg_recipe_ingredient->waste_percent = !empty($value[2]) ? $value[2] : 0;
                //             $mfg_recipe_ingredient->quantity = !empty($value[3]) ? $value[3] : 0;
                //             $mfg_recipe_ingredient->save();

                //             $MfgRecipe_update=MfgRecipe::where('id',$mfg_recipe->id)->first();
                //             $MfgRecipe_update->final_price +=$value[4];
                //             $MfgRecipe_update->total_quantity +=$value[3];
                //             $MfgRecipe_update->save();
                //         }
                //     }
                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    if ($value[0] != null) {
                        ...
                        if ($query->count() > 0) {
                            $is_valid =  false;
                            $error_msg = "Duplicate Product on row no. $row_no";
                            $duplicateRows[] = $row_no; // Store the duplicate row number
                            continue; // Skip current iteration and move to the next row
                        }
                
                        $mfg_recipe = new MfgRecipe;
                        $mfg_recipe->product_id = $product_id->id;
                        $mfg_recipe->variation_id = $variation_id->id;
                        $mfgRecipes[] = $mfg_recipe;
                    } else {
                        if ($value[1] != null) {
                            $variation_ids = Variation::where('sub_sku', $value[1])->first();
                            ...
                            $mfg_recipe_ingredient->waste_percent = !empty($value[2]) ? $value[2] : 0;
                            $mfg_recipe_ingredient->quantity = !empty($value[3]) ? $value[3] : 0;
                            $mfgRecipeIngredients[] = $mfg_recipe_ingredient;
                        }
                    }
                }
                
                // Save records after the loop completes
                if (empty($duplicateRows)) {
                    // Insert mfg_recipe records
                    foreach ($mfgRecipes as $mfg_recipe) {
                        $mfg_recipe->save();
                    }
                    // Insert mfg_recipe_ingredient records
                    foreach ($mfgRecipeIngredients as $mfg_recipe_ingredient) {
                        $mfg_recipe_ingredient->mfg_recipe_id = $mfg_recipe->id;
                        $mfg_recipe_ingredient->save();
                    ...
                    $error_msg = "Duplicate product rows found: " . implode(', ', $duplicateRows);
                }
            ...
            $output = ['success' => 1,
                            'msg' => __('Recipe Import Successfully')
                        ];
            DB::commit();
            return redirect('/manufacturing/recipe')->with('status', $output);
        } catch (\Exception $e) {
            DB::rollBack();
            \Log::emergency("File:" . $e->getFile(). "Line:" . $e->getLine(). "Message:" . $e->getMessage());
            $output = ['success' => 0,
            ...
            return redirect('/manufacturing/recipe')->with('notification', $output);
        }
    }
