<script type="text/javascript">
       $(document).on('click','.btn-uploadCSV-recipe',function(){
	...
	$(document).ready( function () {
	    ...
	    var is_stock_adj = "{{ request('type') }}";
	    var myurl = "";
	    if(is_stock_adj == 'stock_adjustment'){
            myurl= '{{action("\Modules\Manufacturing\Http\Controllers\ProductionController@multiproduction_index",["type" => "stock_adjustment"])}}';
        }else{
            myurl= '{{action("\Modules\Manufacturing\Http\Controllers\ProductionController@multiproduction_index")}}';
        }
        multiproductions_table = $('#multiproductions_table').DataTable({
	        processing: true,
	        serverSide: true,
	        aaSorting: [[0, 'desc']],
	        ajax: {
	        ...
	        fnDrawCallback: function(oSettings) {
	            __currency_convert_recursively($('#multiproductions_table'));
	        }
	    });

	    stock_adj_table = $('#stock_adj_table').DataTable({
	        processing: true,
	        serverSide: true,
	        aaSorting: [[0, 'desc']],
	        ajax: {
	            url: '{{action("\Modules\Manufacturing\Http\Controllers\ProductionController@multiproduction_index")}}',
	            data: function(d) {
	        ...
	        fnDrawCallback: function(oSettings) {
	            __currency_convert_recursively($('#stock_adj_table'));
	        }
	    });
	...
	$(document).on('shown.bs.modal', '#recipe_modal', function(){
        ...
        $(this).find('#select_cat').select2();
        $(this).find('.variation_id').select2();
	});
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result.success == true) {
						...
						multiproductions_table.ajax.reload();
                    } else {
                        toastr.error(result.msg);
                    }
                },
...
$(document).on('change','#choose_product_form #select_cat',function(){
    var cat_id = $(this).val();
    $.ajax({
        method: 'get',
        url: "/manufacturing/get_product/" + cat_id,
        dataType: 'json',
        success: function(result) {
            console.log(result);
            $('.variation_id').html('<option disabled selected>Select Product</option>');
    	    $.each(result, function (index, value) {
    			if(value.name!=null)
    			{
    			  $('.variation_id').append('<option value="' + value.id + '">' + value.name + '</option>');
    			}else{
    			}
            });
        },
