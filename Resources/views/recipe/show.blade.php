		...
		<div class="modal-header">
		    <button type="button" class="close no-print" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
		      <h4 class="modal-title" id="modalTitle">{{$recipe->variation->full_name}}</h4>
		      	<br>
      		 <h4>Gross Weight:{!! $recipe->variation->product->weight !!}</h4>
      		 <h4>Net Weight:{!! $recipe->variation->product->product_custom_field1 !!}</h4>
	    </div>
							...
							@foreach($ingredients as $ingredient)
								...
								@if(empty($ingredient['mfg_ingredient_group_id']))
									<tr>
										...
										<td><span class="" >{{number_format($ingredient['quantity'],3)}}</span> {{$ingredient['unit']}}</td>
