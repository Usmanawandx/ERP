<table class="table table-striped table-th-green text-center ingredients_for_unit_recipe_table">
	<thead>
		<tr>
		...
		@if(!empty($ingredients))
			@foreach($ingredients as $ingredient)
				@if(empty($ingredient['mfg_ingredient_group_id']))
					@include('manufacturing::recipe.ingredient_row_for_production_multy')
				@else
					@php
	...
	<tfoot>
		...
		<tr>
		    <td colspan="4" style="text-align: right;"><strong>Toatal Rate</strong></td>
			<td>
			    <input type="text" readonly name="production_total[]" id="total__cost" style="border: none;"/>
			</td>
		</tr>
	</tfoot>
</table>