...
class MfgRecipe extends Model
{
    ...
    public static function forDropdownAll($business_id, $variation_id = true)
    {
        $recipes = DB::table('variations as v')
                    ->join('products as p', 'v.product_id', '=', 'p.id')
                    ->join('product_variations as pv', 'v.product_variation_id', '=', 'pv.id')
                    ->select(DB::raw('CONCAT(p.name, " (", v.sub_sku, ")") as recipe_name'), 'v.id')
                    ->get();
        return $recipes->pluck('recipe_name', 'id');
    }
    ...
    // public static function forDropdown($business_id, $variation_id = true)
    // {
    //     $recipes = MfgRecipe::join('products as p', 'mfg_recipes.product_id', '=', 'p.id')->select('p.id as product_id','p.name as recipe_name')->get();
    //     // dd($recipes);
    //     // if ($variation_id) {
    //         return $recipes->pluck('recipe_name', 'product_id');
    //     // } else {
    //     //     return $recipes->pluck('recipe_name', 'id');
    //     // }
